...
describe('renderTourPage lastVerified badge', () => {
  const base = {
    title: 'Tour',
    intent: 'Intent.',
    steps: [{ description: 'Step', imagePath: 'a.png', paragraph: 'Paragraph.' }],
  ...
  it('renders a visible badge line under the title when lastVerified is given', () => {
    const page = renderTourPage({ ...base, lastVerified: '2026-08-03 (abc1234)' });
    expect(page).toContain('*Generated by DocSolace · last verified 2026-08-03 (abc1234)*');
    expect(page.indexOf('*Generated by DocSolace')).toBeGreaterThan(page.indexOf('# Tour'));
    expect(page.indexOf('*Generated by DocSolace')).toBeLessThan(page.indexOf('Intent.'));
  });

  it('omits the badge entirely when lastVerified is not given (default, back-compat)', () => {
    const page = renderTourPage(base);
    expect(page).not.toContain('Generated by DocSolace');
  });
});
