...
// v8: a tour page's opt-in `lastVerified` stamp (config.docs.stampVerified,
// same value already written to frontmatter by lib/product.mjs's
// buildFrontmatter) is now also rendered as a visible line under the title
// — frontmatter alone never reaches the reader on Docusaurus's classic doc
// theme, so a project that turned stampVerified on got no actual on-page
// proof despite the flag's name. A project that leaves stampVerified off
// (the default) renders identically to before this version.
export const RENDER_TEMPLATE_VERSION = 8;
...
export function renderTourPage({ title, intent, steps, keepRegionPlaceholder, style, frontmatter, prerequisites, seeAlso, lastVerified }) {
  const {
    primaryViewport,
  ...
  const body = [
    `# ${title}`,
    '',
    // Same opt-in value already stamped into frontmatter (config.docs.
    // stampVerified — see lib/product.mjs's buildFrontmatter), rendered here
    // too because frontmatter alone is invisible on the page itself with
    // Docusaurus's classic doc theme. Omitted entirely (not just blank) when
    // stampVerified is off, matching every other opt-in styling knob in this
    // file.
    ...(lastVerified ? [`*Generated by DocSolace · last verified ${lastVerified}*`, ''] : []),
