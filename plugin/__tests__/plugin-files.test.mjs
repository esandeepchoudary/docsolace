...
describe('skills/document/SKILL.md', () => {
  ...
  it('preserves docs/archive/ in the sidebar when replacing the default autogenerated one', () => {
    // Regression guard: Docusaurus's default {type: 'autogenerated', dirName:
    // '.'} sidebar recurses into every subfolder automatically, including
    // docs/archive/ — the only reason archived tours get a dedicated Archive
    // section for free. Replacing that default with an explicit array (the
    // whole point of this step) stops that recursion, so the explicit array
    // must re-add an autogenerated entry scoped to just "archive", or
    // archived pages silently drop out of the sidebar nav entirely.
    expect(body).toContain("dirName: 'archive'");
    expect(body.toLowerCase()).toContain('label: \'archive\'');
  });
});
