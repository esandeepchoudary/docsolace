...
The fastest path is **`/docsolace:document init-site`** (see
...
about it is DocSolace-specific once it's built.
...
`/docsolace:document init-site` again on an existing site (a "restyle run")
...
`docs/_sidebar.docsolace.json` — a plain, framework-neutral ordering/grouping
