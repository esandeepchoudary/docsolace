...
- **Phase 8 — Orphan detection & archiving. Done.** Phase 7 covers a feature being *added* without a
  tour; this covers the reverse — a tour whose feature was *removed* from the app, which previously
  either went stale silently or started failing capture with a confusing error.
  - **Detection — two mechanical signals, no framework-routing judgment call** (unlike Phase 7's map
    code-review step, which stays a deliberate prompt-driven judgment call): `code-removed` (a tour's
    `code_paths` glob used to resolve to real files, per its last successful generation in
    `state.json`, and no longer does) and `route-unreachable` (none of a tour's `goto` step paths
    appear in `/document map`'s `site-map.json`/`source-routes.json`). Both are plain string/glob
    checks (`plugin/scripts/lib/prune.mjs`'s `findOrphanTours`), unit-testable without a browser —
    exposed standalone as `/document prune`, and folded into `/document map`'s own reconciliation
    (which already produces the crawl/code-review data the second signal needs) as its step 5. The two
    signals aren't equally trusted: `code-removed` checks the committed git tree directly (exact);
    `route-unreachable` checks a crawl/code-review pass that's explicitly best-effort elsewhere in this
    same flow (bounded `maxPages`/`maxDepth`, a skippable profile, a possibly-stale `site-map.json`
    already on disk) — confirmed as a real risk, not hypothetical, by running `prune.mjs` against this
    repo's own leftover `site-map.json` during development and watching it flag two live tours purely
    because that file only had one crawled page on it. `isStrongOrphanSignal` (true only for
    `code-removed`) is what gates autonomous action on this — see below.
  - **Action — archive, never delete.** `plugin/scripts/archive-tour.mjs` flips a tour's `status` to
    the new third value `archived` (alongside `confirmed`/`proposed`). Since there's no subagent
    grounding step involved (just mechanical file moves), the autonomous default archives immediately
    rather than treating it as a hard stop — but **only** for a tour flagged `code-removed`; a
    `route-unreachable`-only candidate is reported, never auto-archived, in either mode. Never deletes
    `tours/<id>.yaml` or any doc content: `docs/<id>.md` (and its images) move to `docs/archive/<id>.md`
    with a banner explaining why, so a wrong call is a one-line revert (flip `status` back, move the
    page back). `docs/archive/_category_.json` files every archived page into its own "Archive" section
    at the bottom of the scaffolded Docusaurus sidebar (autogenerated from folder structure) with no
    manual sidebar wiring needed.
  - **Gate integration.** `status: archived` is checked wherever `draft`/`proposed` already are —
    `lib/drift.mjs`'s `isTourDirty` (never dirty), `capture.mjs`/`generate-docs.mjs` (skip cleanly,
    exit 0), `lib/validate.mjs`'s `validateTour` (no findings — an archived tour's `code_paths`
    matching nothing is expected, not a problem to report).
...
- **P8:** `findOrphanTours` unit-tested against both signals independently and together, including that
  a tour never generated before is never flagged (nothing to have gone stale from) and that
  draft/proposed/already-archived tours are never candidates. `archive-tour.mjs` verified to flip status,
  move the doc page and its images preserving the relative path structure the markdown's image
  references depend on, and write `_category_.json` idempotently. `isTourDirty`/`validateTour` verified
  to treat `archived` exactly like `draft`/`proposed`'s existing skip.
