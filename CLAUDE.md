...
- **`status: archived`** is the third value: a tour whose feature DocSolace
  ...
  **`map` and `prune` are tours-only** — neither dispatches `product-scribe`
  nor runs `generate-product-docs.mjs`, even though drafting or archiving a
  tour changes the tour inventory `_product`'s dirty-check is partly hashed
  against (`lib/product.mjs`'s `computeProductInputsHash`); the product
  pages regenerate only via `product` mode or a no-argument run, per the
  next bullet. So both modes' final report step runs `drift.mjs` and quotes
  its `_product` line, pointing at `/document product` when it comes back
  dirty, rather than silently shipping a PR with a stale overview/sidebar.
